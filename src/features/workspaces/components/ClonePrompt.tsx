...
export function ClonePrompt({
  ...
  return (
    <ModalShell
      ...
      ariaLabel={t("clonePrompt.ariaLabel")}
      onBackdropClick={() => {
        if (!isBusy) {
          onCancel();
        }
    ...
    >
      <div className="ds-modal-title clone-modal-title">{t("clonePrompt.title")}</div>
      <div className="ds-modal-subtitle clone-modal-subtitle">
        {t("clonePrompt.subtitle", { workspaceName })}
      </div>
      <label className="ds-modal-label clone-modal-label" htmlFor="clone-copy-name">
        {t("clonePrompt.copyName")}
      </label>
      <input
        id="clone-copy-name"
        ref={inputRef}
      ...
      <div className="clone-modal-folder-row">
        <textarea
          ...
          value={copiesFolder}
          placeholder={t("clonePrompt.notSet")}
          readOnly
          rows={1}
          wrap="off"
          onFocus={(event) => {
      ...
      {showSuggested && (
        <div className="clone-modal-suggested">
          <div className="clone-modal-suggested-label">{t("clonePrompt.suggested")}</div>
          <div className="clone-modal-suggested-row">
            <textarea
              ...
              rows={1}
              wrap="off"
              aria-label={t("clonePrompt.suggestedAria")}
