...
const PresetStep = memo(function PresetStep({
  ...
  return (
    <div className="review-inline-section">
      ...
      >
        <span className="review-inline-option-title">{t("reviewInline.againstBaseBranch")}</span>
        <span className="review-inline-option-subtitle">{t("reviewInline.prStyle")}</span>
      </button>
      <button
        type="button"
        ...
        disabled={isSubmitting}
      >
        <span className="review-inline-option-title">{t("reviewInline.uncommitted")}</span>
      </button>
      <button
        type="button"
        ...
        disabled={isSubmitting}
      >
        <span className="review-inline-option-title">{t("reviewInline.commit")}</span>
      </button>
      <button
        type="button"
        ...
        disabled={isSubmitting}
      >
        <span className="review-inline-option-title">{t("reviewInline.customInstructions")}</span>
      </button>
    </div>
  );
});
  ...
  return (
    <div className="review-inline-section">
      <div className="review-inline-row">
        ...
        >
          {t("reviewInline.startReview")}
        </button>
      </div>
      <div className="review-inline-hint">{t("reviewInline.pickRecentBranch")}</div>
      <div className="review-inline-list" role="listbox" aria-label={t("reviewInline.baseBranchesAria")}>
        {reviewPrompt.isLoadingBranches ? (
          <div className="review-inline-empty">{t("reviewInline.loadingBranches")}</div>
        ) : branches.length === 0 ? (
          <div className="review-inline-empty">{t("reviewInline.noBranches")}</div>
        ) : (
...
const CommitStep = memo(function CommitStep({
  ...
  return (
    <div className="review-inline-section">
      ...
      <div className="review-inline-hint">{t("reviewInline.pickRecentCommit")}</div>
      <div className="review-inline-list" role="listbox" aria-label={t("reviewInline.commitsAria")}>
        {reviewPrompt.isLoadingCommits ? (
          <div className="review-inline-empty">{t("reviewInline.loadingCommits")}</div>
        ) : commits.length === 0 ? (
          <div className="review-inline-empty">{t("reviewInline.noCommits")}</div>
        ) : (
  ...
  return (
    <div className="review-inline-section">
      ...
      <textarea
        ...
        placeholder={t("reviewInline.placeholder")}
        autoFocus
        rows={6}
      />
    </div>
...
export const ReviewInlinePrompt = memo(function ReviewInlinePrompt({
  ...
  const title = useMemo(() => {
    switch (step) {
      case "baseBranch":
        return t("reviewInline.titleBaseBranch");
      case "commit":
        return t("reviewInline.titleCommit");
      case "custom":
        return t("reviewInline.titleCustom");
      case "preset":
      default:
        return t("reviewInline.titlePreset");
    }
  }, [step, t]);
